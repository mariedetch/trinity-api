...
export const CommandFactory = setSeederFactory(
  Command,
  async (faker, meta?: { user: User }) => {
    ...
    command.createdAt = faker.date.between({ from:"2024-04-01", to:"2025-02-16"});
    ...
    if (command.status != CommandStatus.PAID) {
      command.shipping_charge = faker.number.float({
        min: 1000,
        max: 10000,
        precision: 0.01,
      });
