...
echo IMAGE_NAME=$PROD_IMAGE >> .secure_files/env.prod
echo ECR_URI=$ECR_URI >> .secure_files/env.prod

# Copier les fichiers nécessaires sur l'instance EC2
scp -i .secure_files/trinity-prod-key-pair.pem .secure_files/env.prod ubuntu@$EC2_PROD_PUBLIC_IP:/home/ubuntu/.env
scp -i .secure_files/trinity-prod-key-pair.pem docker-compose.yml ubuntu@$EC2_PROD_PUBLIC_IP:/home/ubuntu/docker-compose.yml

# Exécution des commandes à distance via SSH
ssh -i .secure_files/trinity-prod-key-pair.pem ubuntu@$EC2_PROD_PUBLIC_IP << 'EOF'
